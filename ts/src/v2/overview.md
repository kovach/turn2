...
- `collectBlockedDoAggs` / `collectBlockedChooses` — find `_do-agg` rows lacking an `_agg-result`, and `_choose` rows with unresolved active terms (reading the actor column at terms[3], defaulting `you`); `collectAllBlocked` also pulls in blocked bracket-aggregation rows (kind `aggc`, from comp-aggregate.ts)
