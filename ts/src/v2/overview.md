...
**Key terms:**
- `MomentHandler` — `{ name, run(store, m) → { progress, blocked }, demanded?(store) }`; `run` must be idempotent. `demanded` lists moment tokens with pending work regardless of the frontier — used by demand-style handlers for rows whose left endpoint is *already resolved* (late work through a same-moment `^` chain); those moments are run alongside the frontier without being re-opened, since everything below them is settled
