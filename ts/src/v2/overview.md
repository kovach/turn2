...
- `[ Q | Out = op V ]` bracket aggregation — tokenized as a (possibly multi-line) `aggcomp` token, mini-parsed into a pre-expand `AggComp` atom (query items = plain match atoms or nested `[...]`, chainable with `.` dot notation via the shared `desugarBody` pass; reduction op ∈ count/sum/last). The output pattern left of a top-level `=` is optional: the bare form `[ Q | op V ]` desugars in `desugarBody` (where the rule's used-name set lives) by renaming `V`'s query-side occurrences to a fresh `V_k`, leaving `out` as the Variable `V` — today's behavior as a special case (plans/v2-agg-output-var.md, plans/v2-bracket-aggregation.md)
