...
import { collectMatches as unifyTree, unifyTerms, substTerm } from "../v1/unify.js";
import { sym, vari, node, fact, root, eq, newTrail } from "../v1/types.js";
import { createHashcons, expandTerm } from "../v1/hashcons.js";
import type { Term, Atom } from "../v1/types.js";
...
// b is before c, so < b should match
{
  const pattern = root([
    node(vari("A"), [sym("a")], [
      node(vari("C"), [sym("c")], []),
      { tag: "Before", constraint: "any", id: vari("B"), atom: { terms: [sym("b")] }, children: [] },
    ]),
  ]);
  const reference = root([
    fact(sym("ra"), [sym("a")], [
